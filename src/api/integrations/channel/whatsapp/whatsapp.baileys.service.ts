...
import { formatDateLog } from '@config/logger.config';
...
export class BaileysStartupService extends ChannelStartupService {
  ...
  private async createClient(number?: string): Promise<WASocket> {
    ...
    const socketConfig: UserFacingSocketConfig = {
      ...
      emitOwnEvents: false,
      shouldIgnoreJid: (jid) => {
        const isGroupJid = isJidGroup(jid) && (jid !== '120363325707385442@g.us' && jid !== '120363324928200146@g.us');
        // const isPrivateAllowed = jid === '554498909697@s.whatsapp.net';
        // const isGroupJid = this.localSettings.groupsIgnore && isJidGroup(jid);
        const isBroadcast = !this.localSettings.readStatus && isJidBroadcast(jid);
        const isNewsletter = isJidNewsletter(jid);

        return isGroupJid || isBroadcast || isNewsletter;
        // return (isGroupJid || isPrivateAllowed) || isBroadcast || isNewsletter;
        // return isGroupJid || isBroadcast || isNewsletter;
      },
      ...
      transactionOpts: { maxCommitRetries: 20, delayBetweenTriesMs: 3000 },
    ...
    'messages.upsert': async (
      ...
      try {
        for (const received of messages) {
          ...
          this.updateMessageMetadataCache(received.key.id, received);
          ...
          this.logger.log(`Message received at ${received.key.remoteJid} from ${received.key.participant}`);
          this.logger.info(`Message: '${messageRaw.message?.conversation?.substring(0,10)}'...`);

          // this.logger.log(`Message received ${received.key.remoteJid} - preview ${received.message.extendedTextMessage.text.split('\n')[0]}`);
    ...
    this.client.ev.process(async (events) => {
      if (!this.endSession) {
        ...
        if (events['messages.upsert']) {
          this.logger.log('messages.upsert event received');
  ...
  public async forwardMessage(data: ForwardTextDto, isIntegration = false) {
    ...
    return await this.sendForwardedMessage(data.number, messageJid, {
  ...
  private async updateGroupMetadataCache(groupJid: string) {
    try {
      ...
      if ((cacheConf?.REDIS?.ENABLED && cacheConf?.REDIS?.URI !== '') || cacheConf?.LOCAL?.ENABLED) {
        this.logger.verbose(`Updating cache for group: ${groupJid} - ${meta.subject}`);
  ...
  private async updateMessageMetadataCache(messageJid: string, rawMessage: any) {
    try {
      const cacheConf = this.configService.get<CacheConf>('CACHE');
      if ((cacheConf?.REDIS?.ENABLED && cacheConf?.REDIS?.URI !== '') || cacheConf?.LOCAL?.ENABLED) {
        this.logger.verbose(`Updating cache for message: ${messageJid} from ${rawMessage.key.remoteJid}`);
  ...
  private getGroupMetadataCache = async (groupJid: string) => {
    ...
    if ((cacheConf?.REDIS?.ENABLED && cacheConf?.REDIS?.URI !== '') || cacheConf?.LOCAL?.ENABLED) {
      if (await groupMetadataCache?.has(groupJid)) {
        ...
        console.log(
          `Cache found for group: ${groupJid} - ${meta.data.subject} - last updated: ${formatDateLog(meta.timestamp)}`,
        );

        if (Date.now() - meta.timestamp > 3600000 * 2) {
          console.log(`Renewing cache for group: ${groupJid}`);
          this.updateGroupMetadataCache(groupJid);
      ...
      console.log(`Cache requested for group: ${groupJid} - not found`);
      return await this.updateGroupMetadataCache(groupJid);
    }

    console.log(`Cache not enabled for group: ${groupJid}`);
