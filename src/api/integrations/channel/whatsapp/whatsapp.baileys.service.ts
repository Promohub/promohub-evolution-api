...
export class BaileysStartupService extends ChannelStartupService {
    ...
    private async createClient(number?: string): Promise<WASocket> {
        ...
        const socketConfig: UserFacingSocketConfig = {
            ...options,
            version,
            logger: P({level: this.logBaileys, enabled: false}),
            printQRInTerminal: false,
            auth: {
                creds: this.instance.authState.state.creds,
                keys: makeCacheableSignalKeyStore(this.instance.authState.state.keys, P({
                    level: 'error',
                }) as any),
            },
                    ...
                    if (this.localWebhook.enabled) {
                        if (isMedia && this.localWebhook.webhookBase64) {
                            const buffer = await downloadMediaMessage(
                                ...
                                {},
                                {
                                    logger: P({level: 'error', enabled: false}) as any,
    ...
    private async sendForwardedMessage(
        ...
        try {
            ...
            const messageSent: WAMessage = await this.sendMessage(sender, message, [], true, quoted, messageJid); // Adicionei o messageJid para testar se é útil
    ...
    private getGroupMetadataCache = async (groupJid: string) => {
        ...
        if ((cacheConf?.REDIS?.ENABLED && cacheConf?.REDIS?.URI !== '') || cacheConf?.LOCAL?.ENABLED) {
            ...
            if (await groupMetadataCache?.has(groupJid)) {
                ...
                console.log(`Cache found for group: ${groupJid} - ${meta.data.subject} - last updated: ${formatDateLog(meta.timestamp)}`);
